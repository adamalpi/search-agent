...
class QueryHistorySchema(BaseModel):
    ...
    limit: int = Field(
        default=5, description="Maximum number of recent COMPLETED tasks to retrieve."
    )
...
def _query_analysis_history_func(tool_input: str) -> str:
    ...
    industry_filter = None  # Default filter

    try:
        # Attempt to parse the input string as JSON
        data = json.loads(tool_input)
        # Validate the parsed data using the Pydantic schema
        validated_data = QueryHistorySchema.model_validate(data)
        limit = validated_data.limit
        industry_filter = validated_data.industry_filter
        logging.info(
            f"Tool: Parsed arguments - limit={limit}, industry='{industry_filter}'"
        )

    except json.JSONDecodeError:
        logging.warning(
            f"Tool: Input is not valid JSON: {tool_input}. Using default parameters."
        )
        ...
        logging.warning(
            f"Tool: Input validation failed: {e}. Using default parameters."
        )
        ...
        logging.error(
            f"Tool: Unexpected error processing input '{tool_input}': {e}",
            exc_info=True,
        )
    ...
    logging.info(
        f"Tool: Querying analysis history (limit={limit}, industry='{industry_filter}')"
    )
    try:
        tasks: List[Dict[str, Any]] = query_tasks(
            limit=limit, industry_filter=industry_filter
        )

        if not tasks:
            filter_msg = f" for industry '{industry_filter}'" if industry_filter else ""
            return (
                f"No completed analysis tasks found{filter_msg} matching the criteria."
            )
...
if __name__ == "__main__":
    ...
    print(
        query_analysis_history_tool.run('{"limit": 2, "industry_filter": "Automotive"}')
    )  # Input must be JSON string
    print("\nQuerying tasks for 'NonExistentIndustry':")
    print(
        query_analysis_history_tool.run('{"industry_filter": "NonExistentIndustry"}')
    )  # Input must be JSON string
    print("\nQuerying with invalid JSON:")
    print(
        query_analysis_history_tool.run('{"limit": "not-an-int"}')
