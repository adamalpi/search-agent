...
# Use absolute imports assuming 'sustainability_research_agent' is in the Python path
...
try:
    with open("prompts/map_prompt.txt", "r") as f:  # Updated path
        map_prompt_template = f.read()
    map_prompt = PromptTemplate.from_template(map_prompt_template)

    with open("prompts/combine_prompt.txt", "r") as f:  # Updated path
    ...
    logging.info(
        "Successfully loaded custom map/combine prompts and created summarize chain for graph."
    )
except Exception as e:
    logging.error(
        f"Failed to load map/combine prompts or create summarize chain in graph_builder: {e}",
        exc_info=True,
    )
    print("FATAL: Could not set up summarization chain. Exiting.")
    exit(1)
...
def identify_companies(state: GraphState) -> GraphState:
    ...
    try:
        ...
        companies = [
            name.strip() for name in company_response_content.split(",") if name.strip()
        ]
...
def download_and_extract(state: GraphState) -> GraphState:
    ...
    for company, url in report_urls.items():
        if company in extracted_texts:
            continue  # Skip if already processed
        if url is None or url.startswith("Error"):
            logging.warning(
                f"Skipping download/extraction for {company} due to missing/error URL: {url}"
            )
        ...
        try:
            ...
            if extracted_text.startswith("Error") or extracted_text.startswith(
                ...
                logging.warning(
                    f"Text extraction failed/empty for {company}: {extracted_text}"
                )
                extracted_texts[company] = extracted_text  # Store warning/error
            else:
                logging.info(
                    f"Successfully extracted text for {company} (length: {len(extracted_text)})."
                )
                extracted_texts[company] = extracted_text
        except Exception as e:
            logging.error(
                f"Error during download/extraction for {company}: {e}", exc_info=True
            )
...
def summarize_reports(state: GraphState) -> GraphState:
    ...
    for company, text in extracted_texts.items():
        if company in individual_summaries:
            continue  # Skip if already processed
        if (
            text is None
            ...
            logging.warning(
                f"Skipping summarization for {company} due to previous error or empty text."
            )
        ...
        if os.path.exists(cache_filepath):
            logging.info(
                f"Cache hit for {company} summary: Loading from {cache_filepath}"
            )
            try:
                ...
                logging.warning(
                    f"Failed to read cached summary for {company} from {cache_filepath}: {e}. Will regenerate."
                )
        ...
        try:
            ...
            try:
                ...
                logging.error(
                    f"Failed to save summary to cache for {company} at {cache_filepath}: {e}"
                )
...
def synthesize_trends(state: GraphState) -> GraphState:
    ...
    try:
        ...
        with open("prompts/synthesis_prompt.txt", "r") as f:  # Updated path
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n--- Synthesis Result ---")
        print(
            final_state.get("synthesis_result")
            or final_state.get("error_message", "Unknown error")
        )
