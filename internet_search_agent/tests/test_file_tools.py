...
from internet_search_agent.file_tools import (
    _is_valid_url,
    _url_to_filename,
    download_pdf,
...
def test_is_valid_url():
    ...
    assert _is_valid_url("https://example.com/path?query=1") == True
    assert _is_valid_url("ftp://example.com") == False # Wrong scheme
    assert _is_valid_url("example.com") == False # No scheme
    assert _is_valid_url("http://") == False # No netloc
...
def test_download_pdf_cache_hit(mocker):
    ...
    mock_get = mocker.patch('internet_search_agent.file_tools.requests.get')
...
def test_download_pdf_cache_miss_success(mocker):
    ...
    mock_get = mocker.patch('internet_search_agent.file_tools.requests.get', return_value=mock_response)
...
def test_extract_text_from_pdf_success(mocker):
    ...
    with open(filepath, "w") as f: f.write("not real pdf")
    ...
    mocker.patch('internet_search_agent.file_tools.PdfReader', return_value=mock_reader_instance)
...
def test_download_pdf_tool_run(mocker):
    ...
    mock_download = mocker.patch('internet_search_agent.file_tools.download_pdf')
...
def test_extract_pdf_text_tool_run(mocker):
    ...
    mock_extract = mocker.patch('internet_search_agent.file_tools.extract_text_from_pdf')
