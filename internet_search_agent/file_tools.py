...
def download_pdf(url: str) -> str:
    ...
    try:
        ...
        if 'application/pdf' not in content_type:
             logging.warning(f"Content-Type for {url} is '{content_type}', not 'application/pdf'. Attempting download anyway.")
...
def extract_text_from_pdf(local_filepath: str) -> str:
    ...
    try:
        ...
        if not text:
            logging.warning(f"No text could be extracted from PDF: {local_filepath}. It might be image-based or protected.")
